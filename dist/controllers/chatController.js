...
exports.chatController = {
    async listConversations(_req, res) {
        const result = await (0, db_1.query)(`SELECT c.id, c.title, c.description, c.is_group, c.admin_subrole,
              c.avatar_initials,
       ...
       ORDER BY c.title ASC`);
        if (result.rowCount === 0) {
            for (const c of DEFAULT_CONVERSATIONS) {
                await (0, db_1.query)(`INSERT INTO conversations (title, is_group, admin_subrole, avatar_initials)
           VALUES ($1, TRUE, $2, $3)`, [c.title, c.admin_subrole, c.initials]);
            }
            const refreshed = await (0, db_1.query)(`SELECT c.id, c.title, c.description, c.is_group, c.admin_subrole,
    ...
    async getMessages(req, res) {
        ...
        const result = await (0, db_1.query)(`SELECT m.id, m.conversation_id, m.sender_id, m.content, m.status, m.created_at,
    ...
    async sendMessage(req, res) {
        ...
        const result = await (0, db_1.query)(`INSERT INTO messages (conversation_id, sender_id, content, status)
       VALUES ($1, $2, $3, 'SENT')
       RETURNING id, conversation_id, sender_id, content, status, created_at`, [conversationId, req.user.sub, content.trim()]);
